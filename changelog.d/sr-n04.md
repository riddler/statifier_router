...
- `StatifierRouter.Delivery` delivers for `:never` bindings, returning `{:dropped, binding_id, :no_execution}` when the address has no row, and for `:always_new` bindings, creating one execution per delivery with no address row; neither returns `{:error, :not_implemented}` any more.
